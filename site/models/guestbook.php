...
class Tz_guestbookModelGuestbook extends JModelForm{
    ...
    function populateState(){
        ...
        $this->setState('hstatus',$h_status);
   ...
   */
    ...
    */
    function getInsertcontent(){
        ...
        $tz_status  = $this->getState('hstatus');
        ...
        switch($website){
            case'Your website':
                $website="";
                 break;
            default:
                $website = strip_tags(htmlspecialchars($website));
                break;
        }
        if(isset($name) && !empty($name) && isset($email) && !empty($email)   && isset($content) && !empty($content)) {
            ...
            $sql = 'INSERT INTO #__comment values(null,"'.$name.'","'.$email.'","'.$title.'","'.$content.'",
                 "'.$publich.'","'.$datee.'","'.$tz_status.'","'.$website.'","'.$id.'")';
            $db -> setQuery($sql);
            $num_row = $db->query();
  ...
  */
    function ajax(){
        ...
        require_once(JPATH_COMPONENT.DS.'views'.DS.'guestbook'.DS.'view.html.php');
        ...
        $tstatus = $state->get('shownow');
        $view -> assign('hstatus',$tstatus);
